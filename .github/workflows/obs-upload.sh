...
obs_sync_files()
{
	local suite="$1" dir="$2" commit_message="$3"
	local remote_list=$WORK_DIR/remote-files.txt

	echo "Syncing files to OBS project/package: $OBS_PROJECT/$suite"

	$OSC list -l $OBS_PROJECT $suite > $remote_list
	...
	do
		if [ ! -f "$remote_file" ]
		then
			echo "D    $suite/$remote_file ..."
			osc_delete_file $OBS_PROJECT $suite "$remote_file"
			do_commit=true
		fi
	done < $remote_list
	...
	do
		...
		if $do_upload
		then
			echo "A    $suite/$local_file ..."
			osc_add_file $OBS_PROJECT $suite "$local_file"
			do_commit=true
		fi
	done
	...
	then
		...
		osc_commit $OBS_PROJECT $suite "$commit_message"
	else
...
$comment"
	...
	obs_sync_files $suite $upload_dir "$commit_message"
	echo ' '
done
