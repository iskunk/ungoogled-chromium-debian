...
do_conversion()
{
	local id="$1" deb_version="$2" uc_tag="$3"
	local conv_dir=$WORK_DIR/convert/$id
	...
	# NOTE: Nested output groups not yet supported
	...
	local files=
	  files="$id/ungoogled-chromium_$uc_version.dsc"
	files+=" $id/ungoogled-chromium_$uc_version.debian.tar.xz"
	ofiles=" $id/ungoogled-chromium_$ups_version.orig.tar.xz"
...
do
	...
	IFS="$tab" read deb_suite deb_version uc_tag < $todo

	echo "::group::Build DEB($deb_suite, $deb_version) + UC($uc_tag)"

	do_conversion "$deb_suite" "$deb_version" "$uc_tag"
